...
# --- UI / CARD BUILDER ---
def build_card(title, template, lines, actions=None):
...
def send_card(chat_id, card):
    req = im.CreateMessageRequest.builder().receive_id_type("chat_id").request_body(
        im.CreateMessageRequestBody.builder().receive_id(chat_id).msg_type("interactive").content(json.dumps(card, ensure_ascii=False)).build()
    ).build()
...
def card_user_menu(user):
    ...
    return build_card(f"✅ {user['name']}", "green", 
                      [f"**Staff No:** {user['staffNo']}", f"**สถานะ:** {'🟢 เปิดใช้งาน' if is_enable else '🔴 ปิดใช้งาน'}", "", "**เลือกดำเนินการ:**"], 
                      [
...
def event_handler():
    if request.method in ["GET", "HEAD"]: return jsonify({"status": "ok"}), 200
    ...
    # 1. จัดการกดปุ่ม (Actions)
    if event_type == "card.action.trigger":
        event = data.get("event", {})
        chat_id = event.get("context", {}).get("open_chat_id")
        choice = event.get("action", {}).get("value", {}).get("choice")
        
        # เชื่อม Logic การกดปุ่ม
        actions = {"1": "เปิดใช้งาน", "2": "ปิดใช้งาน", "3": "Reset PDA", "4": "Reset JMS"}
        if choice in actions:
            send_card(chat_id, build_card("⚙️ ดำเนินการ", "blue", [f"กำลังสั่ง {actions[choice]}..."]))
        elif choice == "5":
    ...
    # 2. จัดการข้อความปกติ (Messages)
        ...
        if "ค้นหา id" in text:
            ...
            if res.get("found"): send_card(chat_id, card_user_menu(res))
            else: send_card(chat_id, build_card("❌ ไม่พบข้อมูล", "red", ["ไม่พบ Staff No ในระบบ"]))
